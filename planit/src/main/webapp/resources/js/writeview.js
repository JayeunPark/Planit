...
function makemap(){
   ...
   var mapContainer = document.getElementById('map'+cnt), // 지도를 표시할 div 
       mapOption = { 
       center: new kakao.maps.LatLng(37.49998231489462, 127.03541966047779), // 지도의 중심좌표
       level: 3 // 지도의 확대 레벨
   };
   ...
   function removeaccrows(e){
      const tr = e.composedPath()[3];
       ...
       if(totalAmount[budget]){
    	   ...
    	   //총액이 0이면 - 붙이지 않기위함
           if((totalAmount[spending])==0){
               e.composedPath()[5].children[2].children[1].children[3].children[0].value =(totalAmount[spending]); 
...
// 천의 단위로 콤마찍기 정규식 적용함
   ...
   //별에 id class부여하기
       ...
       star.children[0].setAttribute("class","star");
   ...
   function alone(){
       ...
       if(friend_content != null){
          friend_content.value = '';
          for (var i = 0; i < friends.length; i++) {
             const friendCheckDiv = document.getElementById("friendDiv"+i);
             const addfriBtn = document.getElementById("addfriBtn");
             const hiddenmessage = document.getElementById("hiddenmessage");
             friendCheckDiv.style.display = 'block';
             friends[i].checked = false;
                 addfriBtn.style.display = 'inline';
   ...
   function addfri() {
      ...
      const authority = document.getElementById("authority_content");
      // 취득한 속성 만큼 루프
      if(authority.value != '혼자하기'){
         let flag = false;
         for (let i = 0; i < friends.length; i++) {
            const labelVal = document.querySelector('label[for="friend'+i+'"]');
            const friendCheckDiv = document.getElementById("friendDiv"+i);
            const friendlabel = labelVal.getAttribute('id');
            // 속성중에 체크 된 항목이 있을 경우
               if (friends[i].checked) {
                  /*arr.push(friends[i].value);*/
                  newarr.push(labelVal.innerHTML);
                  friendCheckDiv.style.display = 'none';
         ...
         if(!flag){
            const addfriBtn = document.getElementById("addfriBtn");
            addfriBtn.style.display = 'none';
            const hiddenmessage = document.getElementById("hiddenmessage");
            hiddenmessage.setAttribute('class', '');
         }
      } else{
         alert("혼자하기일때는 친구 추가가 안됩니다.");
      }
          // 결과를 표시
      if(newarr.length == 0){
         document.getElementById("friend_content").value = remain;
      } else {
         document.getElementById("friend_content").value = remain + "," + newarr;
      }
   ...
   friend_content.addEventListener('click', function() {
      ...
      if(document.querySelector('label[for="friend0"]') == null){
         const addfriBtn = document.getElementById("addfriBtn");
         addfriBtn.style.display = 'none';
         const hiddenmessage = document.getElementById("hiddenmessage");
         hiddenmessage.setAttribute('class', '');
    ...
    function makefood() {
         ...
         const categorywrap_food = document.createElement("div");
          ...
          clonefd.children[0].children[0].setAttribute('value','food-'+cnt);
          clonefd.children[0].children[0].setAttribute('name','cnt');
          clonefd.style.display = 'block';
          clonefd.style.position = 'relative';
...
//          console.log(clonefd.children);    
           ...
           const cl_fd_header = clonefd.children[0].children[1];
//           console.log(cl_fd_header.children)

//           // #fd_header - #fd_header_lt -> id + cnt, name생성 + cnt
           const cl_fd_header_lt =cl_fd_header.children[0];
...
//           // #fd_header - #fd_header_rt - #star -> id + cnt, name 생성 +cnt
           const cl_fd_star = cl_fd_header_rt.children[0].children[1];
...
//           // #fd_cont - #fd_text -> id + cnt, name + cnt
           const cl_fd_text = clonefd.children[0].children[2].children[0];
          ...
          const star = cl_fd_header_rt.children[0];
          star.setAttribute("id","star");
          star.setAttribute("class","star");
          star.children[0].setAttribute("id","starSpan"+cnt);
          star.children[0].style.width=0;
          ...
          clonefd.children[1].addEventListener("click",deleteCate);
          cnt++;
       }
